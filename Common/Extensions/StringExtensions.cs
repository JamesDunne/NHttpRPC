    ...
    public static class StringExtensions
    {
        ...
        public static bool CaseInsensitiveTrimmedEquals(this string self, params string[] compare)
        {
            for (int i = 0; i < compare.Length; ++i)
            {
                ...
                if (String.Equals(self.Trim(), (compare[i] ?? string.Empty).Trim(), StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
        ...
        public static bool CaseInsensitiveEquals(this string self, params string[] compare)
        {
            for (int i = 0; i < compare.Length; ++i)
            {
                ...
                if (String.Equals(self, compare[i], StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
        ...
        /// <summary>
        /// Formats the current string using functions depending on if the string is non-empty or empty.
        /// </summary>
        /// <param name="self"></param>
        /// <param name="format">Called to format the string if it is non-empty.</param>
        /// <param name="empty">Called to format the string if it is empty.</param>
        /// <returns></returns>
        public static string NullOrEmptyFormat(this string self, Func<string, string> format, Func<string> empty)
        {
            if (self == null) return empty();
            if (self.Length == 0) return empty();
            if (self.Trim().Length == 0) return empty();
        ...
        /// <summary>
        /// Replaces named token placeholders in the form of {TOKEN_NAME} with their replacement contents
        /// found by calling the <paramref name="getTokenReplacementText"/> function provided.
        /// Case-sensitivity of the token names is determined by the <paramref name="getTokenReplacementText"/>
        /// function.
        /// </summary>
        /// <param name="templateText"></param>
        ...
        public static bool CanParseInt32(this string value)
        {
            ...
            int tmp;
            if (Int32.TryParse(value, out tmp)) return true;
        ...
        public static string HTML(this string text)
        {
            ...
            foreach (char c in text)
            {
                if (c == '&')
                    ...
                    sb.AppendFormat("&#{0};", ((int)c).ToString());
                else sb.Append(c);
            }
            return sb.ToString();
        }

        public static string HTMLAttribute(this string text)
        {
            if (text == null) return null;
            if (text.Length == 0) return text;
